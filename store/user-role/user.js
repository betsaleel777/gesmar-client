...
export const actions = {
  ...
  async restaurer({ dispatch }, id) {
    const requete = await this.$axios.get('api/parametres/users/restore/' + id)
    dispatch('getAll')
    return { message: requete.data.message }
  },
  async ajouter({ dispatch }, payload) {
    const requete = await this.$axios.post(
      'api/parametres/users/store',
      payload
    )
    dispatch('getAll')
    return { message: requete.data.message }
  ...
  async autoriser({ dispatch }, payload) {
    const requete = await this.$axios.post(
      'api/parametres/users/autoriser',
      payload
    )
    dispatch('getAll')
    return { message: requete.data.message }
  ...
  async notifications({ dispatch }, payload) {
    const requete = await this.$axios.post(
      'api/parametres/users/notifications',
      payload
    )
    dispatch('getAll')
    return { message: requete.data.message }
