...
export const actions = {
  ...
  async getTrashAll({ commit }) {
    commit('SET_FACTURE', [])
    const requete = await this.$axios.get('api/finances/factures/loyers/trashed')
    commit('SET_FACTURE', requete.data.factures)
  },
  ...
  async supprimer({ dispatch }, id) {
    const requete = await this.$axios.delete('api/finances/factures/loyers/' + id)
    dispatch('getAll')
    return { message: requete.data.message }
  },

  async restaurer({ dispatch }, id) {
    const requete = await this.$axios.patch('api/finances/factures/loyers/restore/' + id)
    dispatch('getAll')
    return { message: requete.data.message }
  },

  async ajouter({ dispatch }, payload) {
    const requete = await this.$axios.post('api/finances/factures/loyers/store', payload)
    dispatch('getPaginate')
    return { message: requete.data.message }
  },
}
