...
export const actions = {
  ...
  async getTrashAll({ commit }) {
    ...
    const requete = await this.$axios.get(
      'api/finances/factures/annexes/trashed'
    )
    commit('SET_FACTURE', requete.data.factures)
  },
  ...
  async modifier({ dispatch }, payload) {
    const requete = await this.$axios.put(
      'api/finances/factures/annexes/' + payload.id,
      payload
    )
    dispatch('getAll')
  ...
  async supprimer({ dispatch }, id) {
    const requete = await this.$axios.delete(
      'api/finances/factures/annexes/' + id
    )
    dispatch('getAll')
    return { message: requete.data.message }
  },
  async restaurer({ dispatch }, id) {
    const requete = await this.$axios.get(
      'api/finances/factures/annexes/restore/' + id
    )
    dispatch('getAll')
    return { message: requete.data.message }
  },
  async ajouter({ dispatch }, payload) {
    const requete = await this.$axios.post(
      'api/finances/factures/annexes/store',
      payload
    )
    dispatch('getAll')
    return { message: requete.data.message }
