...
export const actions = {
  ...
  async getTrashAll({ commit }) {
    ...
    const requete = await this.$axios.get(
      'api/finances/factures/equipements/trashed'
    )
    commit('SET_FACTURE', requete.data.factures)
  },
  async getOne({ commit }, id) {
    const requete = await this.$axios.get(
      'api/finances/factures/equipements/' + id
    )
    return requete.data
  },
  async modifier({ dispatch }, payload) {
    const requete = await this.$axios.put(
      'api/finances/factures/equipements/' + payload.id,
      payload
    )
    dispatch('getAll')
  ...
  async restaurer({ dispatch }, id) {
    const requete = await this.$axios.get(
      'api/finances/factures/equipements/restore/' + id
    )
    dispatch('getAll')
    return { message: requete.data.message }
  },
  async ajouter({ dispatch }, payload) {
    const requete = await this.$axios.post(
      'api/finances/factures/equipements/store',
      payload
    )
    dispatch('getAll')
    return { message: requete.data.message }
