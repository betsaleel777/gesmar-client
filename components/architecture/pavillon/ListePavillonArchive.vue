    ...
    <b-card aria-hidden="true" header="Utilisateurs Archivées">
      <b-card-text>
        ...
        <b-table
          ...
          :fields="fields"
          responsive
          empty-text="Aucun pavillons archivés"
          show-empty
          :filter="filter"
          :filter-debounce="500"
          @filtered="onFiltered"
        >
          ...
          <template #cell(code)="data"> {{ data.item.code }}-{{ data.item.site.nom }} </template>
        ...
        <ConfirmationModal
          ...
          :nom="dialogData.nom"
          modal-id="pavillonConfirmationArchive"
          action="architecture/pavillon/restaurer"
...
export default {
  ...
  computed: {
    ...mapGetters('architecture/pavillon', ['pavillons']),
  },
  methods: {
    ...mapActions('architecture/pavillon', ['getTrashAll']),
    imprimer() {},
    dialoger({ id, nom }) {
      this.dialogData.nom = nom
      this.dialogData.id = id
      this.dialogData.modal = true
      this.$bvModal.show('pavillonConfirmationArchive')
    },
