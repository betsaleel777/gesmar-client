  ...
  <div>
    ...
    <b-card aria-hidden="true" header="Liste des commerciaux">
      <b-card-text>
        ...
        >
          ...
          <template #cell(option)="data">
            ...
            <a type="button" class="mr-1" @click="transferer(data.item)">
              <feather
                :title="`transférer à ${data.item.name}`"
                type="refresh-ccw"
                size="20"
                stroke="orange"
              />
            </a>
            <a type="button" class="mr-1" @click="attribuer(data.item.id)">
        ...
        />
        <ShowAttributionBordereauModal v-if="show.modal" v-model="show.modal" :commercial="show.commercial" />
